...
# -*- coding: utf-8 -*-
"""
小飯碗：一碗牛丼長出手腳、碗身上兩顆眼睛。

一碗牛丼長出手腳、碗身上兩顆眼睛。矮一點、寬一點，
才不會看起來只是「人物換個頭」—— 站在一起要一眼分得出誰是誰。

配色沿用人物那組（person_sprite.PAL），兩個角色才像同一個世界的。
碗身白、碗緣 OKAWARI 紅、上面褐色肉片配綠蔥花，照品牌簡報那張牛丼照。

網格 20 欄 × 17 列。人物是 18×28，所以小飯碗明顯矮胖。
"""
...
# 人物用 sit 當睡姿，小飯碗用 sleep。兩邊都給一個 sit 別名，
# 渲染器才不用去記誰有哪個姿勢。
